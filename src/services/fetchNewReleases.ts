export async function fetchNewReleases(pageSize = 5) {
  ...
  const apiUrl = `https://api.rawg.io/api/games?key=${apiKey}&dates=${today},${today}&ordering=-released&page_size=${pageSize}`;
